...
const { isDebugToolsEnabled } = require('./src/debug');
const { createSoundManager } = require('./src/sound');
const {
...
function boot() {
  ...
  soundManager = createSoundManager(wx);
...
function confirmRestartLevel() {
  if (!state || state.completed || !wx || typeof wx.showModal !== 'function') {
    return;
  }
...
function startMenuTiltTracking() {
  if (
    ...
    !wx ||
    typeof wx.startAccelerometer !== 'function' ||
    typeof wx.onAccelerometerChange !== 'function'
  ) {
    return;
  }
  ...
  try {
    wx.onAccelerometerChange(menuAccelerometerHandler);
...
function stopMenuTiltTracking() {
  ...
  if (wx && typeof wx.offAccelerometerChange === 'function' && menuAccelerometerHandler) {
    wx.offAccelerometerChange(menuAccelerometerHandler);
  }

  if (wx && typeof wx.stopAccelerometer === 'function') {
